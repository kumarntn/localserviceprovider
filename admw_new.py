...
st=f.getvalue("st")
wid=f.getvalue("wid")
if st!=None and wid!=None:
	q="""update workers set status='%s' where wkr_id='%s'"""%(st,wid)
	cur.execute(q)
	conn.commit()
	print("""
		<script>
			alert("Request %sed Successfully !!");
			location.href="admw_new.py";
		</script>
		"""%(st))

q="""select * from workers where status='Pending'"""
cur.execute(q)
r=cur.fetchall()
cnt=0
print("""
...
for i in r:    
	...
	print("""                          
			<tr>
				...
				<td><a href="admw_new.py?st=%s&wid=%s">Accept</a><br><a href="admw_new.py?st=%s&wid=%s">Reject</a></td>
				
			</tr>"""%(cnt,fn,fn,i[3],i[4],i[5],i[6],i[7],i[8],fn1,fn1,'Accept',i[1],'Reject',i[1]))
print("""
       </tbody>
                                </table>
                            </div>

...
q="""select * from services where wkr_id='%s' and status!='Finished'"""%(r[1])
cur.execute(q)
r1=cur.fetchall()
cnt=0

if sid!=None:
	q="""update services set status='%s' where srvc_id='%s'"""%('Finished',sid)
	cur.execute(q)
	conn.commit()
	print("""
		<script>
			alert("Status updated successfully!!");
			location.href="w_wo_new.py?id=%s";
		</script>
		"""%(id))
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <table id="bootstrap-data-table" class="table table-striped table-bordered">
                                    ...
                                    <tbody>""")
for i in r1:  
	q="""select contact from users where usr_id='%s'"""%(i[5])
	cur.execute(q)
	r2=cur.fetchone()
	q="""select type from payment where srvc_id='%s'"""%(i[1])
	cur.execute(q)
	r3=cur.fetchone()
	if r3!=None:
		pay=r3[0]
	...
	print("""                          
			<tr>
				...
				<td>%s</td>
				<td>%s</td>
				<td>If finished <a href="w_wo_new.py?id=%s&sid=%s" style="color:blue;">Click here</a>?</td>
				
			</tr>"""%(cnt,i[1],i[2],i[3],i[4],i[6],r2[0],i[9],pay,id,i[1]))
print("""
       </tbody>
                                </table>
                            </div>

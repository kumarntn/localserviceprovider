...
if sub!=None:
	...
	if profile.filename:
		...
		q="""update users set profile='%s',password='%s' where id=%s"""%(fn1,pwd,id)
		cur.execute(q)
		conn.commit()
	else:
		q="""update users set password='%s' where id=%s"""%(pwd,id)
		cur.execute(q)
		conn.commit()
	print("""
		<script>
			alert("Profile Changed successfully!!");
			location.href="uviewprofile.py?id=%s";
		</script>
		"""%(id))
				...
				<div class="animated fadeIn">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body card-block">
                            <form action="uchangeprofile.py" method="post" enctype="multipart/form-data">

...
r=cur.fetchone()
fn='files/'+r[9]
q="""select * from services where wkr_id='%s' and status='%s'"""%(r[1],'Finished')
cur.execute(q)
r1=cur.fetchall()
cnt=0
print("""
...
for i in r1:  
	...
	cur.execute(q)
	r3=cur.fetchone()
	q="""select * from feedbacks where srvc_id='%s' and wkr_id='%s'"""%(i[1],r[1])
	cur.execute(q)
	r4=cur.fetchone()
	cnt=cnt+1
	if r4[8]!="":
		...
		td="""<a href="contractor_feedback.py?id=%s&sid=%s" style="color:blue;">Click here...</a>"""%(id,i[1])
	print("""                          
			<tr>
				<td>%s</td>
				<td>%s</td>
			...
			</tr>"""%(cnt,i[1],i[2],i[3],i[4],i[6],i[9],r3[0],td))
print("""
       </tbody>
                                </table>
                            </div>
